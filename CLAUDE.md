...
- **`window.AndroidPrint.print()`** — triggers Android's native `PrintManager` (via `bridge.getWebView().createPrintDocumentAdapter()`), which itself offers "Save as PDF" as a destination. Used by `src/utils.ts`'s `printHtmlDocument()` for the Report page's Print/PDF buttons.
  ...
  - The report's in-app "✕ Close" button (outside the iframe, in the main document) would otherwise show up in the captured print/PDF output too — hidden via an injected `<style>@media print{.wms-print-close{display:none!important}}</style>` scoped to that button's class.
